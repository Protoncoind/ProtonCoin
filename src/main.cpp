...
#if defined(NDEBUG)
# error "Protoncoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x505b83737809807cc8686704fefb107a028ff389eeabefb4c291fba82a462e86");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Protoncoin: starting difficulty is 1 / 2^12
...
{
    // Protoncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
// MODIFICACION DE PRUEBA JAIMEN URBINA
int64 static GetBlockValue(int nHeight, int64 nFees)
{
   ...
   // nSubsidy >>= (nHeight / 50); // Protoncoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}
// MODIFICACION DE PRUEBA JAIMEN URBINA

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Protoncoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Protoncoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Protoncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Protoncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe14a41a198934b009665e8f6ec45ece2409fb136b4fbb7ca1d12ef5f9d7da72f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Nacimiento de Protoncoin en Barquisimeto Estado Lara";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 13 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0416a367817e2eadb88861a45c3cc84906abfc56e154ba4fe6c0cfaee34916acca863849dab6b80d323caf865eb55408459d37b9ee89f1daf4a8cec23cf6f3bcd5") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1597584942;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087386066;

        if (fTestNet)
        {
            block.nTime    = 1597584912;
            block.nNonce   = 385524877;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x6514b5184542440089f516d30540499e08af90fdf7ae6a1eecc32cff83797d5b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc2, 0xba, 0xd2 }; // Protoncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ProtoncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ProtoncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ProtoncoinMiner(CWallet *pwallet)
{
    printf("ProtoncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("protoncoin-miner");
    ...
    try { loop {
        ...
        printf("Running ProtoncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ProtoncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ProtoncoinMiner, pwallet));
}
